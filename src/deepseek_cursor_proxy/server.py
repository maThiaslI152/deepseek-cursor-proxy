...
class RequestMetrics:
    ...
    def cache_hit_pct(self) -> str:
        ...
        if not total:
            return "-"
        return f"{self.cache_hit / total:.0%}"
...
class DeepSeekProxyHandler(BaseHTTPRequestHandler):
        ...
        try:
            with response:
                ...
                if not sent_response.sent:
                    ...
                    return
                spinner.stop()
                log_stats_summary(sent_response.usage, self.metrics)
                self._finish_trace(
                    trace,
                    "completed",
                    http_status=upstream_status,
    ...
    def _proxy_streaming_response(
        ...
        recording_scopes: list[tuple[str, list[dict[str, Any]]]] = []
        if record_response_contexts:
            recording_scopes.extend(record_response_contexts)
        active_scope = conversation_scope(request_messages, cache_namespace)
        recording_scopes.append((active_scope, request_messages))
        seen: set[str] = set()
        response_contexts: list[tuple[str, list[dict[str, Any]]]] = []
        for scope, msgs in recording_scopes:
            if scope and scope not in seen:
                seen.add(scope)
                response_contexts.append((scope, msgs))
...
def default_config_path() -> Path:
    return Path("deepseek-cursor-proxy.yaml")
...
def log_stats_summary(
    ...
    proxy_rate = metrics.cache_hit_pct if metrics else "-"
    LOG.info(
        "└ stats   prompt=%s output=%s reasoning=%s api_cache=%s proxy_cache=%s",
