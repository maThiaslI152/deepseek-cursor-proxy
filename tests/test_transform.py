...
class CrossModeAndModelTests(unittest.TestCase):
    ...
    def test_namespace_keys_provide_recall_across_conversations(self) -> None:
        ...
        hit_kind = str(portable_prepared.reasoning_diagnostics[-1]["hit_kind"])
        self.assertIn(
    ...
    def test_recovered_response_is_recorded_under_pre_recovery_scope(self) -> None:
        ...
        # Also verify the namespace-level tool_call key for cross-conversation recall
        self.assertEqual(
            self.store.get(
                f"namespace:{first_recovered.cache_namespace}:tool_call:call_new"
            ),
            "Need the new lookup.",
        )
